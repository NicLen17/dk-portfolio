...
interface AboutPreviewContentProps {
  data?: SanityAboutSettings | null;
}

export function AboutPreviewContent({ data }: AboutPreviewContentProps) {
  const { lang, t } = useLanguage();
  const ref = useRef<HTMLDivElement>(null);
  const isInView = useInView(ref, { once: true, margin: "-15%" });

  const sectionLabel = resolveLocale(data?.sectionLabel, lang) || t.about.sectionLabel;
  const words = data?.previewWords?.length
    ? data.previewWords.map((w, idx) => ({
        text: w.endsWith(".") ? w : `${w}.`,
        className: defaultWords[idx]?.className || defaultWords[0].className,
      }))
    : defaultWords;

  const bio = resolveLocale(data?.previewBio, lang) || t.about.introP1;
  const bio2 = t.about.introP2;
      ...
      <div className="max-w-[1600px] mx-auto px-6 md:px-10 lg:px-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-start">
          <div className="lg:col-span-6 min-w-0">
            <SectionLabel className="mb-6">{sectionLabel}</SectionLabel>
            <motion.h2
