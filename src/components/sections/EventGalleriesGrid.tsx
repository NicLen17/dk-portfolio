...
interface EventGalleriesGridProps {
  events?: SanityEventListItem[] | null;
}

export function EventGalleriesGrid({ events }: EventGalleriesGridProps) {
  ...
  const sectionLabelText = lang === "ES" ? "Galerías de Eventos" : "Event Galleries";
  const headingText = lang === "ES" ? ["ENCUENTRA TU MOMENTO.", "ADQUIERE TU FOTO."] : ["FIND YOUR MOMENT.", "OWN YOUR PHOTO."];
  const descText = lang === "ES" 
    ? "Después de un evento, encuentra tus fotos, previsualiza cada imagen y contáctanos para adquirir tu momento."
  ...
  const openEventsText = lang === "ES" ? "VER EVENTOS" : "OPEN EVENTS";
  const previewText = lang === "ES" ? "PREVISUALIZAR" : "PREVIEW GALLERY";
                    ...
                    >
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-6">
                        <p className="text-xs font-body uppercase tracking-[0.2em] text-neutral-400 mb-1">
                          {event.photoCount} {lang === "ES" ? "Fotos" : "Photos"}
                        </p>
